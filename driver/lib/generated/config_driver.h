...
const int DRIVER_PANEL_DISPLAY_SPEED_REFRESH_DELAY_MS = 100; // Delay in millis between speed value updates on the display. When the modes changes the value will be updated immediately.
const int DRIVER_THROTTLE_READING_REACTION_THRESHOLD_RPM = 22; // Threshold in RPM reacting to target speed changes read from the throttle leaver. If the difference between reading and current target speed (as absolute value) exceeds this threshold, the target speed will be updated. This only affects the manual control mode.
